document.addEventListener('DOMContentLoaded', function () {
    ...
    const customIcon = L.icon({
        iconUrl: '/gps2/icons/marker-icon-2x.png',
        shadowUrl: '/gps2/icons/marker-shadow.png',
    ...
    document.getElementById('reconstruccionBtn').addEventListener('click', function (event) {
        ...
        console.log("Reconstrucción de recorrido activada para GPS2.");

        // Detener la actualización automática de las últimas ubicaciones
        if (typeof stopUpdating === 'function') {
            stopUpdating();
            console.log("Actualización automática detenida para reconstrucción.");
        }

        // Cargar el archivo HTML para el modal
        fetch('/gps2/reconstruccion2.html')
            ...
            .then(html => {
                ...
                fetch('/coordinates2/getUniqueRutas')
                ...
                document.getElementById('reconstruirBtn').addEventListener('click', function () {
                    ...
                    console.log(`Reconstruyendo con id_ruta: ${idRuta} y fecha: ${fechaSeleccionada}`);

                    // Limpiar todos los marcadores actuales
                    clearMarkers();

                    // Realizar la consulta enviando el id_ruta y la fecha
                    fetch(`/coordinates2/reconstruirRecorrido?id_ruta=${idRuta}&fecha=${fechaSeleccionada}`)
                        .then(response => response.json())
                        .then(coordenadas => {
