...
router.post('/', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'INSERT INTO coordenadas2 (latitude, longitude, timestamp, vpn_validation, id_ruta, battery) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
...
router.get('/latest-coordinates', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT id, latitude, longitude, id_ruta, timestamp, vpn_validation, battery FROM coordenadas2
...
router.get('/getUniqueRutas', async (req, res) => {
    try {
        const result = await pool.query("SELECT DISTINCT id_ruta FROM coordenadas2");
...
router.get('/', (req, res) => {
    res.status(200).send('Atlas sabe donde estas en coordenadas2!');
});
